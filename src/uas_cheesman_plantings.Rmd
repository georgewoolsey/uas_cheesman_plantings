...
plot that on top of the original
...
terra::plot(
  ...
  , col =  grDevices::colorRampPalette( RColorBrewer::brewer.pal(9, "Greens") )(100)
  , main = "ExGR > 0"
  , axes = F
  , plg = list(
    title = "ExGR"
...
looking good. let's use `lidR::locate_trees` with a LMF to detect the greenest pixel (as the center of the tree) to use in a MCWS ;D
...
crowns %>% 
  ...
  terra::vect() %>% 
  terra::crop(aoi) %>% 
  terra::plot(add = T, col = NA, border = "cyan", lwd = 1.5)
```
