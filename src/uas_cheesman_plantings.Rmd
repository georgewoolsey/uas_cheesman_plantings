...
dem_files <- list.files(path = "../data/usgs_dem", pattern = "*\\.tif$", full.names = TRUE)

# Read them into a terra collection pointer list
dem_list <- lapply(dem_files, terra::rast)

# Combine the rasters into a single unified mosaic dataset
# terra::sprc acts as a fast SpatRasterCollection wrapper
compiled_dem_1m <- terra::mosaic(terra::sprc(dem_list))
names(compiled_dem_1m) <- "elevation"

# 3. Derive Core Terrain Layers ----
# Slopes and aspects must be calculated in radians for McCune's math equations
slope_rad  <- terra::terrain(compiled_dem_1m, v = "slope", unit = "radians")
aspect_rad <- terra::terrain(compiled_dem_1m, v = "aspect", unit = "radians")

# Extract the continuous Y coordinates (Latitude) across the grid canvas
# We transform them to latitude degrees if the native projection uses meters
lonlat_grid <- terra::crds(compiled_dem_1m, df = TRUE, na.rm = FALSE)
lat_vector  <- lonlat_grid$y

# Convert the localized planar coordinates to absolute latitude degrees
# (Adjust this conversion if your native raster is already in EPSG:4326)
lat_rast <- terra::rast(compiled_dem_1m, vals = lat_vector)
lat_rad  <- lat_rast * (pi / 180)

# 4. Compute Topographic Heat Load Index (HLI) ----
# Folded Aspect handles the SW-NE physical symmetry axis (McCune & Keon 2002)
# Folded Aspect = |pi - |Aspect - 5pi/4|| 
folded_aspect <- abs(pi - abs(aspect_rad - (5 * pi / 4)))

# Implement McCune & Keon (2002) Equation 2 (For True Heat Load Index)
hli_1m <- exp(-0.291 + 
                (0.814 * cos(lat_rad) * cos(slope_rad)) - 
                (2.311 * sin(lat_rad) * sin(slope_rad)) - 
                (0.611 * cos(folded_aspect) * sin(slope_rad)) - 
                (0.354 * sin(folded_aspect) * sin(slope_rad)))

names(hli_1m) <- "hli"

# 5. Create Coarse Grain Grid (5-meter Scale) ----
# Aggregate cuts computational weight while preserving macro-topography averages
compiled_dem_5m <- terra::aggregate(compiled_dem_1m, fact = 5, fun = "mean")
hli_5m          <- terra::aggregate(hli_1m, fact = 5, fun = "mean")

# Stack the multi-scale structural metrics together for zonal profiling
terrain_stack_1m <- c(compiled_dem_1m, slope_rad * (180 / pi), hli_1m)
...
set.seed(42)
kmeans_model <- base::kmeans(clustering_data, centers = 3, nstart = 25)
