...
| :--- | :--- | :--- | :--- |
| **Cool / Mesic** | `0.00` to `0.25` | Steep Northeast Slopes | Minimized solar exposure, high moisture retention, and buffered soil temperatures. Protects shade-tolerant understory flora and acts as a climate refugium. |
| **Cool-Moderate** | `> 0.25` to `0.50` | Mild North/East Slopes & Deep Ravines | Low-to-moderate solar loading. Evapotranspiration rates remain stable, supporting consistent vegetative growth and reliable seed germination. |
| **Warm-Moderate** | `> 0.50` to `0.75` | Flat Areas / Mild South & West Slopes | Baseline-to-high solar exposure. Typical open-sky growing conditions where photosynthesis is highly active but bounded by standard rainfall cycles. |
| **Hot / Xeric** | `> 0.75` to `1.00` | Steep Southwest Slopes | Maximized afternoon solar radiation. Elevated thermal stress and rapid soil drying curves create a harsh, drought-prone microclimate selecting for hardy, heat-tolerant plant species. |

define those groupings
...
what if we group by those HLI ranges we noted above?

```{r}
# viridis::cividis(9) %>% scales::show_col()
pal_hli_grp <- viridis::cividis(9)[c(1,3,7,9)]
...
let's check those candidate sample locations centered on surviving tree point locations
...
known_trees_sf <-
  ...
  dplyr::mutate(year_tree_id = dplyr::row_number()) %>% 
  dplyr::ungroup()
```
...
terra::plot(terra::vect(cheesman_sf), col = NA, border = "black", add = T)
terra::plot(terra::vect(known_trees_sf), col = "cyan", add = T)
...
```{r}
candidate_centers <- known_trees_sf %>%  
  dplyr::group_by(planting_year) %>% 
  dplyr::slice_head(n = 10) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(plot_id = dplyr::row_number())

# 6. Generate 1-Acre Square Sample Units ----
# Buffer the point centers outward by 31.8 meters using square cap boundaries
# This outputs a perfectly uniform square measuring exactly 1 acre (4,046 m2)
candidate_plots_sf <- sf::st_buffer(
  x = candidate_centers, 
  dist = 31.8, 
  endCapStyle = "SQUARE"
)
...
clustering_inputs <- final_candidate_plots |> 
  sf::st_drop_geometry() |> 
...
final_candidate_plots <- final_candidate_plots |> 
  dplyr::mutate(topo_group_cohort = base::as.factor(kmeans_model$cluster))

print(final_candidate_plots)
...
# Stack the multi-scale structural metrics together for zonal profiling
terrain_stack_1m <- c(dem_1m_rast, slope_rad_rast * (180 / pi), hli_1m_rast)
names(terrain_stack_1m) <- c("elevation", "slope_deg", "hli")

# 

# 7. Extract Exact Zonal Topographic Statistics ----
# exactextractr::exact_extract computes perfect area-weighted means for overlapping edge cells
zonal_metrics <- exactextractr::exact_extract(
  x = terrain_stack_1m, 
  y = candidate_plots_sf, 
  fun = "mean",
  append_cols = c("plot_id", "planting_year")
) %>% 
  dplyr::rename(
    mean_elevation = mean.elevation,
    mean_slope     = mean.slope_deg,
    mean_hli       = mean.hli
  )
...
final_candidate_plots <- candidate_plots_sf %>% 
