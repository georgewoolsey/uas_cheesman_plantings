...
dem_fnm_temp <- "../data/usgs_dem/dem_1m_rast.tif"

# dem_1m_rast <-
#   list.files(path = dirname(dem_fnm_temp), pattern = "*\\ArapahoRooseveltPikeNF_D23.tif$", full.names = TRUE, recursive = F) %>%
#   .[1] %>%
#   terra::rast()

if(!file.exists(dem_fnm_temp)){
  dem_list <- 
    list.files(path = dirname(dem_fnm_temp), pattern = "*\\ArapahoRooseveltPikeNF_D23.tif$", full.names = TRUE, recursive = F) %>% 
    unique() %>% 
  ...
  dem_1m_rast <- terra::mosaic(terra::sprc(dem_list), fun = "mean", filename = dem_fnm_temp, overwrite=F)
  
}else{
  dem_1m_rast <- terra::rast(dem_fnm_temp)
}
# name
...
f1_temp <- file.path(dirname(dem_fnm_temp), "lat_degrees_rast.tif" )
...
f2_temp <- file.path(dirname(dem_fnm_temp), "lat_rad_rast.tif" )
...
# ?terra::terrain
f3_temp <- file.path(dirname(dem_fnm_temp), "terrain_rad_rast.tif" )
f31_temp <- file.path(dirname(dem_fnm_temp), "slope_rad_rast.tif" )
f32_temp <- file.path(dirname(dem_fnm_temp), "aspect_rad_rast.tif" )
if(
  !file.exists(f3_temp) ||
...
# # hillshade?
# ?terra::shade
f4_temp <- file.path(dirname(dem_fnm_temp), "shade_rast.tif" )
...
# terra::plot(shade_rast, col = grDevices::gray(0:100/100), legend = F, axes = F, mar=c(1,1,1,4))
# terra::plot(
#   terrain_rad_rast$aspect
#   , col = harrypotter::hp(n=100, option = "ravenclaw")
#   , add = T
#   , alpha = 0.4
#   , plg = list(
#      title = "aspect (rad)"
...
# ?terra::extend
# identical(terra::ext(terrain_rad_rast$slope), terra::ext(dem_1m_rast))
f5_temp <- file.path(dirname(dem_fnm_temp), "hli_1m_rast.tif" )
...
# Assume 'known_trees_sf' contains spatial point geometries and a 'planting_year' column
# We sample 10 candidates per planting year
